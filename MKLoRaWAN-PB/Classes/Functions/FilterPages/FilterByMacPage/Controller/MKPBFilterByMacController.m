...
@interface MKPBFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKPBFilterByMacModel *dataModel;

@end

@implementation MKPBFilterByMacController

- (void)dealloc {
    NSLog(@"MKPBFilterByMacController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Filter by MAC";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PB", @"MKPBFilterByMacController", @"pb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKPBFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPBFilterByMacModel alloc] init];
    }
    return _dataModel;
}
