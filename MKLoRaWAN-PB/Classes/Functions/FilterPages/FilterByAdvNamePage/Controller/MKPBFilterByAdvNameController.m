//
//  MKPBFilterByAdvNameController.m
...
@interface MKPBFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKPBFilterByAdvNameModel *dataModel;

@end

@implementation MKPBFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKPBFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKPBFilterEditSectionHeaderViewModel *headerModel = [[MKPBFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
...
- (void)saveDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        ...
        [nameList addObject:SafeStr(cellModel.textFieldValue)];
    }
...
- (void)loadSection1Datas {
    for (NSInteger i = 0; i < self.dataModel.nameList.count; i ++) {
        MKTextFieldCellModel *cellModel = [[MKTextFieldCellModel alloc] init];
        cellModel.index = i;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PB", @"MKPBFilterByAdvNameController", @"pb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKPBFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPBFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
