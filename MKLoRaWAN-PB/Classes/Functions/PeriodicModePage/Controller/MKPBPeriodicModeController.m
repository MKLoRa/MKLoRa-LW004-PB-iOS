...
@interface MKPBPeriodicModeController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKPBPeriodicModeModel *dataModel;

@end

@implementation MKPBPeriodicModeController

- (void)dealloc {
    NSLog(@"MKPBPeriodicModeController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PB", @"MKPBPeriodicModeController", @"pb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKPBPeriodicModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPBPeriodicModeModel alloc] init];
    }
    return _dataModel;
}
