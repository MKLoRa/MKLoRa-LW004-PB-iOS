...
@interface MKPBGeneralController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKPBGeneralPageModel *dataModel;

@end

@implementation MKPBGeneralController

- (void)dealloc {
    NSLog(@"MKPBGeneralController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Device Mode
        MKPBDeviceModeController *vc = [[MKPBDeviceModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Auxiliary Operation
        MKPBAuxiliaryController *vc = [[MKPBAuxiliaryController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //3-Axis Settings
        MKPBAxisSettingsController *vc = [[MKPBAxisSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Bluetooth Settings
        MKPBBleSettingsController *vc = [[MKPBBleSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PB", @"MKPBGeneralController", @"pb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKPBGeneralPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPBGeneralPageModel alloc] init];
    }
    return _dataModel;
}
